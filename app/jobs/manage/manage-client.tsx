...
export default function ManageJobsClient({
  autoApproveEnabled: initialAutoApproveEnabled,
  initialPagination,
  initialPosts,
  initialStatusFilter,
  isStaff,
  ...
  useEffect(() => {
    setStatusFilter(initialStatusFilter.toLowerCase());
  }, [initialStatusFilter]);

  useEffect(() => {
    setAutoApproveEnabled(initialAutoApproveEnabled);
  }, [initialAutoApproveEnabled]);

  const navigate = useCallback(
    (page: number, nextStatus: string) => {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen px-4 py-8">
      <div
        aria-busy={isPending}
        className={`max-w-6xl w-full space-y-6 transition-opacity ${isPending ? "opacity-70" : ""}`}
      >
        <Card>
            ...
            {isStaff && (
              <div className="flex items-center gap-3">
                ...
                <Switch
                  ...
                  isDisabled={isTogglingAutoApprove || isPending}
          ...
          <CardBody>
            <Tabs
              ...
              onSelectionChange={(key) => {
                const nextStatus = key as string;

                setStatusFilter(nextStatus);
                navigate(1, nextStatus);
              }}
            >
