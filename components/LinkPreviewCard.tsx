...
/**
 * Detects the platform from a URL and returns icon + brand color.
 */
...
export const LinkPreviewCard = memo(function LinkPreviewCard({
  ...
  const platform = getPlatformInfo(sourceUrl);
  ...
  return (
    <div className="w-full px-5 py-6">
      <a
        ...
        aria-label={translations.externalContent}
          ...
          {isLoading ? (
            ...
            <>
              <h4 className="font-semibold text-foreground line-clamp-2 mb-1">
                {ogData?.title || postTitle || sourceUrl}
              </h4>
              {ogData?.description && (
